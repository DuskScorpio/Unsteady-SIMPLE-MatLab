...
timeEnd = 50; % termination time
nSteps = round(timeEnd / dt); % number of time steps
maxIterations = 200;
maxResidual = 1e-8; % outer loop residual tolerance
...
while ~steadyReached && n < nSteps
    ...
    while residual > maxResidual && iterations < maxIterations
        ...
        residual = sqrt(sum(b_new.^2, 'all')); % compute residual as RMS of the source term
    ...
    %% End of time step
    u_diff = max(abs(u_old(:) - u(:)));
    v_diff = max(abs(v_old(:) - v(:)));
...
fprintf(fileID, 'Time Step (dt): %.4f\n', dt);
