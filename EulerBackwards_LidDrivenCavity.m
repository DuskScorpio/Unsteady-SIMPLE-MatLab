...
nSteps = round(timeEnd / dt); % number of time steps
maxIterations = 200; % max number of SIMPLE loops
...
%% Residual Plot Setup
figure('Name', 'Real-time Convergence', 'NumberTitle', 'off');
hRes = animatedline('Color', 'r', 'LineWidth', 1.5);
set(gca, 'YScale', 'log'); % CFD residuals must be on a log scale
grid on;
xlabel('Total SIMPLE Iterations');
ylabel('RMS Residual (Mass Continuity)');
title('Solver Convergence');

%% Time marching
while ~steadyReached && n < nSteps
    ...
    while residual > maxResidual && iterations < maxIterations
        ...
        for i = 2:numCellsY + 1
            for j = 3:numCellsX + 1
                ...
                v_n_interp = 0.5 * (v(i, j) + v(i, j - 1));
                v_s_interp = 0.5 * (v(i + 1, j) + v(i + 1, j - 1));
        ...
        for i = 3:numCellsY + 1
            for j = 2:numCellsX + 1
                ...
                u_e_interp = 0.5 * (u(i, j + 1) + u(i - 1, j + 1));
                u_w_interp = 0.5 * (u(i, j) + u(i - 1, j));
        ...
        addpoints(hRes, totalIterations + iterations, residual);
    ...
    if maxDiff < steadyTolerance
        fprintf('Steady-state reached at time %d (max diff = %.3)\n', n * dt, maxDiff);
        steadyReached = true;
    end
