%% Plot L2 Error Comparison: Euler vs Leap-Frog
% This script reads L2 error data from Excel and creates bar charts
% comparing RMSE between Euler and Leap-Frog methods

function PlotL2ErrorComparison()
    close all;
    
    % Specify the Excel file
    excelFile = "C:\Study\Y4S3\FYP\L2 Error Extracted.xlsx";
    ...
    for idx = 1:length(re_numbers)
        ...
        euler_rmse = data{:, cols(2)};
        lf_rmse = data{:, cols(3)};
        
        % Create figure
        fig = figure('Name', sprintf('L2 Error Comparison - Re=%d', re), 'NumberTitle', 'off', "Theme", "light");
        fig.Position = [100 100 900 600];
        
        % Create bar chart
        x = categorical(string(cfl_values));
        y_data = [euler_rmse, lf_rmse];
        ...
        ylabel('Total $L_2$-Norm of Velocity Deviation', 'Interpreter', 'latex')
        legend('Euler', 'Leapfrog', 'Interpreter','latex', 'FontSize', 22, 'Location', 'best');
        grid on;
